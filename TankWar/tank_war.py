...
class TankWar:
    ...
    def __draw_map(self):
        ...
        if self.current_level == 1:
            ...
            for y in range(len(Settings.MAP_TWO)):
                for x in range(len(Settings.MAP_TWO[y])):
                    if Settings.MAP_TWO[y][x] == 0:
                        continue
                    wall = Wall(Settings.WALLS[Settings.MAP_TWO[y][x]], self.screen)
                    wall.rect.x = x*Settings.BOX_SIZE
                    wall.rect.y = y*Settings.BOX_SIZE
                    if Settings.MAP_TWO[y][x] == Settings.RED_WALL:
                        wall.type = Settings.RED_WALL
                    elif Settings.MAP_TWO[y][x] == Settings.IRON_WALL:
                        wall.type = Settings.IRON_WALL
    ...
    def __check_collide(self):
        ...
        for wall in self.walls:
            ...
            if pygame.sprite.collide_rect(self.hero, wall):
                # 不可穿越墙
                if wall.type == Settings.RED_WALL or wall.type == Settings.IRON_WALL or wall.type == Settings.SEA_WALL or wall.type == Settings.BOSS_WALL:
        ...
        for enemy in self.enemies:
            for bullet in enemy.bullets:
                if pygame.sprite.collide_rect(bullet, self.hero):
                    bullet.kill()
                    self.hero.kill()
        if not self.enemies:  # 如果敌人组为空，说明敌人全灭
            self.__next_level()
